...
import { GroqProvider } from "../agents/providers/groq-provider.js";
...
const profileName = process.env.AGENT_PROFILE || "groqbot";
const profile = await loadAgentProfile(profileName);
if (!profile) {
  throw new Error(`Groq agent profile not found: ${profileName}.ttl`);
}
...
if (!fileConfig?.nickname || !fileConfig?.xmpp?.username) {
  throw new Error("Groq agent profile is missing nickname or XMPP username");
}
...
const baseXmpp = {
  ...
  resource: process.env.GROQ_RESOURCE
};
...
const provider = new GroqProvider({
  ...
  model: (fileConfig.groq && fileConfig.groq.model) || process.env.GROQ_MODEL || "llama-3.3-70b-versatile",
  nickname: BOT_NICKNAME,
  systemPrompt: fileConfig.groq?.systemPrompt,
  systemTemplate: fileConfig.groq?.systemTemplate,
  ...
  handlers[LANGUAGE_MODES.MODEL_NEGOTIATION] = new ModelNegotiationHandler({
    ...
    onPayload: async ({ payload, roomJid, stanza }) => {
      ...
      if (messageType === MFR_MESSAGE_TYPES.MODEL_CONTRIBUTION_REQUEST) {
        if (!sessionId) {
          logger.warn?.("[GroqBot] MFR contribution request missing sessionId");
          return null;
        }

        if (!targetRoom) {
          logger.warn?.("[GroqBot] Cannot determine target room for MFR contribution");
          return null;
        }

        logger.info?.(`[GroqBot] Generating MFR contribution for session ${sessionId}`);
        const rdf = await provider.handleMfrContributionRequest(payload);
        if (rdf && rdf.trim() && modelFirstRdfHandler && negotiator?.xmppClient) {
          logger.info?.(`[GroqBot] Sending ${rdf.length} bytes of RDF to ${targetRoom}`);
      ...
      if (messageType === MFR_MESSAGE_TYPES.SESSION_COMPLETE) {
        if (!sessionId || !targetRoom) {
          logger.warn?.("[GroqBot] SessionComplete missing sessionId or target room");
          return null;
        }

        const solution = Array.isArray(payload.solutions) ? payload.solutions[0] : payload.solution;
        if (!solution) {
          logger.warn?.("[GroqBot] SessionComplete missing solution payload");
          return null;
        }
...
async function start() {
  console.log(`Starting GroqBot "${BOT_NICKNAME}"`);
...
start().catch((err) => {
  console.error("Failed to start GroqBot:", err);
  process.exit(1);
});
