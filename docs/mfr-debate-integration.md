...
**Status:** ✅ Phase 1 Complete - Feature available with `MFR_ENABLE_DEBATE=true`

## How to Try This Out

### Step 1: Enable the Debate Feature

Add this to your `.env` file:

```bash
MFR_ENABLE_DEBATE=true
```
...
You should see:
1. **Coordinator** announces debate session started with 60-second window
...
4. After 60 seconds (or if Chair detects consensus), the debate concludes
5. System proceeds to normal MFR entity discovery phase
...
### Phase 2: Automated Consensus (PARTIALLY COMPLETE)

**Implemented:**
1. ✅ Basic consensus detection in Chair agent (majority voting on tool names)
2. ✅ Chair can detect when consensus reached via `detectToolConsensus()`

**Remaining:**
1. ⏳ Chair automatically notifies Coordinator when consensus reached (currently manual via "tool consensus" query)
2. ⏳ Coordinator extracts selected tools from consensus message
3. ⏳ Coordinator filters contribution requests to selected agents only

**Deliverable:** Automatic tool selection based on debate (partial - consensus detection works, automatic filtering pending)
...
- Feature available with `MFR_ENABLE_DEBATE=true`
